...
Ray Camera::generate_ray(double x, double y) const {
  ...
  float rhFov = hFov * M_PI / 180.0;
  float rvFov = vFov * M_PI / 180.0;
  Vector3D dir = c2w * Vector3D((2*x - 1)*tan(rhFov / 2), (2*y - 1) * tan(rvFov / 2), -1);
  dir.normalize();
  Ray out = Ray(this->pos,dir);
  out.min_t = 0; out.max_t = std::numeric_limits<double>::infinity();
  return out;
}
