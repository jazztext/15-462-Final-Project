...
Spectrum PathTracer::trace_ray(const Ray &r, bool includeLe = false) {
  ...
  if (!bvh->intersect(r, &isect)) {
    ...
    if (this->envLight == NULL) return Spectrum(0,0,0);
    else {
      Vector3D tmp;
      float d,p;
  ...
  for (SceneLight *light : scene->lights) {
    ...
    for (int i=0; i<num_light_samples; i++) {
      ...
      if (bvh->intersect(shadow, &inters)) continue;
  ...
  if (UniformGridSampler2D().get_sample().x < killP) return L_out;
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  double h = frameBuffer.h, w = frameBuffer.w;
//  double m = max(w, h);
//  int dx = (m - w)/2, dy = (m - h)/2;
  if (num_samples > 1) {
    for (int i=0;i<num_samples;i++) {
      Vector2D p = gridSampler->get_sample();
      Ray r = camera->generate_ray((x + p.x) / w, (y + p.y) / h);
      total += trace_ray(r, true);
    }
    total *= (1.0 / num_samples);
  } else {
